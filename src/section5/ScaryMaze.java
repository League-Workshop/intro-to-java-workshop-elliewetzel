...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	public void mouseMoved(MouseEvent e) {
		...
		int endColor = -16647520;
		//8. If the mouse falls off the path (if it is on the background)
		if(maze.getRGB(mouseX, mouseY) == backgroundColor) {
			scare();
		}
				// call the scare method
		
		//13. If the mouse is on the end color
		if(maze.getRGB(mouseX, mouseY) == endColor) {
				// pop up a message to tell them they won
		JOptionPane.showMessageDialog(null, "Your have won the maze!");
		}
	}
